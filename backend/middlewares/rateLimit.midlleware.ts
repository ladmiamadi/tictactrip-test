...
const checkRateLimit = (req: Request, res: Response, next: NextFunction) => {
    ...
    if(req.body.trim() === '') return res.status(400).json({error: "There is no text to justify"});

    const numberOfWords = countWords(req.body);
    const currentCount = parseInt(req.cookies.rateLimit) || 0;

    if (currentCount + numberOfWords > limit) return res.status(402).json({ error: 'Payment Required' });
